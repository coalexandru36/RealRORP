    ...
    if weapon then
      ...
      if weapon == "body_armor" then -- get player weapons to not rebuy the body
        ...
        if new_weight <= vRP.getInventoryMaxWeight(user_id) then
          ...
          vRPclient.notify(player,{"~w~Nu ai spatiu in buzunar"})
        end
          ...
          vRP.prompt(player,lang.gunshop.prompt_ammo({choice}),"",function(player,amount)
            ...
            if amount >= 0 and amount ~= nil and amount ~= "" then
              if amount <= 250 then
                ...
                vRPclient.notify(player,{"~w~Poti cumpara doar pana la 250 de gloante!"})
              end
            else
              vRPclient.notify(player,{"~w~Valoare invalida!"})
            end
          end)
      else
        vRP.prompt(player,lang.gunshop.prompt_ammo({choice}),"",function(player,amount)
