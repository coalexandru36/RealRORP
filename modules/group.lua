...
function vRP.addUserGroup(user_id,group)
  if not vRP.hasGroup(user_id,group) then
    ...
    if ngroup then
      ...
      exports.oxmysql:query("UPDATE vrp_users SET theJob = @value WHERE id = @user_id",{['@user_id'] = user_id,['@value'] = group})
    end
  end
end
    ...
    for k,v in pairs(selectors) do
      ...
      if gcfg and menu then
        ...
        if gcfg == "Joburi Ilegale" then
          ...
          vRPclient.addMarkerNames(source,{x,y,z+0.350, "~w~JOB ~b~Realism", 4, 1.1})          
          vRPclient.addMarkerSign(source,{21,x,y,z-1.65,0.50,0.60,0.60,255,255,255,150,150,1,false,0})
        end
