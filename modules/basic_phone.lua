...
function vRP.sendServiceAlert(sender, service_name,x,y,z,msg)
  ...
  if service then
    ...
    for k,v in pairs(vRP.rusers) do
      ...
      -- check user
    if(service_name == "Politia Romana") and player ~= nil then
    ...
    elseif(service_name == "Mecanic") and player ~= nil then
			if vRP.isUserInFaction(tonumber(k), "Mecanic") then
				table.insert(players,player)
			end
		else
    ...
    vRPclient.getPosition(player,{},function(x,y,z)
      vRP.prompt(player,lang.phone.service.prompt(),"",function(player, msg)
          ...
          TriggerClientEvent('playPhoneAnim',player)
			    ...
			    vRPclient.notify(player,{"~r~Mesaj gol."})
	      end
      end)
    end)
  end
...
local function ch_announce_alert_police(player,choice)
  ...
  if user_id ~= nil then
    ...
    if vRP.isUserInFaction(user_id,"Politia Romana") or vRP.isUserInFaction(user_id,"DIICOT") or vRP.isUserInFaction(user_id,"SRI") then
      ...
      vRPclient.notify(player, {"Esti Off Duty Nu Ai Acces"})
    end
    else
      vRPclient.notify(player, {lang.common.not_allowed()})
    end
