...
items["bandaj"] = {"Bandaje", "Bandajeaza-te.",nil,0.5,"pocket"}
...
items["secure_card"] = {"Card Malicos", "Pentru jaf!",nil,0.1,"pocket"}
items["AK47"] = {"AK47", "Nu il pierde!",nil,0.1,"pocket"}
items["fibra"] = {"Fibra Optica", "Nu il pierde!",nil,0.1,"pocket"}

items["topor"] = {"Topor", "Nu il pierde!",nil,0.1,"pocket"}
items["trusascule"] = {"Trusa Scule", "Nu il pierde!",nil,0.1,"pocket"}
items["pickaxe"] = {"Tarnacop", "Nu il pierde!",nil,0.1,"pocket"}
items["undita"] = {"Undita", "Nu il pierde!",nil,0.1,"pocket"}
...
timeout_armura = {}
items["body_armor"] = {"Armura","Armura",function(args)
  local choices = {}
  local idname = args[1]

  choices["Equip"] = {function(player,choice,mod)
    local user_id = vRP.getUserId(player)
    if user_id ~= nil then
      if timeout_armura[user_id] == nil then 
        ...
        if vRP.tryGetInventoryItem(user_id, idname, 1, true) then 
          ...
          timeout_armura[user_id] = true 
          SetTimeout(10000,function()
            timeout_armura[user_id] = nil 
          end)
        end
      else
        vRPclient.notify(player,{"Asteapta, ai timeout la armura 10 secunde."})
      end
    end
  end}
		...
		if user_id ~= nil then
			if(vRP.getInventoryItemAmount(user_id,"lighter") > 0)then
				if(vRP.tryGetInventoryItem(user_id, idname, 1, false))then
					...
					vRPclient.notify(player, {"Ti-ai aprins tigara!"})
				else
			...
			choices["Scoate 20 Tigari"] = {function(player,choice,mod)
				...
				if user_id ~= nil then
					if(vRP.tryGetInventoryItem(user_id, idname, 1, false))then
						...
						vRPclient.notify(player, {"Ai scos o tigara din pachet!"})
					end
					vRP.closeMenu(player)
				end
			end}
...
local fptInventoryC = Tunnel.getInterface("fpt-inventory", "vRP");
...
local wammo_choices = function(args)
  ...
  choices["Incarca"] = {function(player,choice)
    ...
    if user_id then
      ...
      vRP.prompt(player, "Cate gloante vrei sa incarci ? (max "..amount..")", "", function(player,ramount)
        local ramount = tonumber(ramount)
        if not ramount then return end;
        local weapon = args[2]:lower();
        fptInventoryC.hasWeaponInHand(player, {weapon}, function(hasWeaponInHand)
          if hasWeaponInHand then -- check if has weapon in hand
            if vRP.tryGetInventoryItem(user_id, fullidname, ramount, true) then
              fptInventoryC.giveAmmo(player, {weapon, ramount}) -- give weapon ammo
              vRP.closeMenu(player)
            end
          end
        end)
