function OGRP_MENU() {
	...
	buildMainMenu = function() {
		$("body").append(
			...
			$("<div>",{"id":"ogrp_mainMenu","class":"ogrp_mainMenu"}).css("display","none").append(
				$("<h1>",{"id":"ogrp_menuLogo","class":"ogrp_menuLogo"}),
	...
	this.open = function(data) {
		...
		//	background-image: url('https://cdn.discordapp.com/attachments/837420661181907015/890284388275879956/logo_RAGE.png');
		//$("#ogrp_menuHeader").html(menuName);
	...
	this.close = function() {
		if(this.opened) {
			...
			if(menuLevel < 0) {
				this.opened = false;
				$("#ogrp_mainMenu").hide();
				$.post("http://vrp/closeTotalMenu", JSON.stringify({}));
			}
